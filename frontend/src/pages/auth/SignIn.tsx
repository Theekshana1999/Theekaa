...
const SignIn: React.FC = () => {
  ...
  // Make phone a string to avoid TS problems and ensure we always send strings
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // Ensure payload are strings
      const payload = { phone: String(phone).trim(), password: String(password) };
      const res: any = await signin(payload).unwrap();

      // Expect backend returns { user, token } — guard defensively
      if (res?.user && res?.token) {
        ...
        console.warn("Signin returned unexpected shape:", res);
      }
    } catch (err: any) {
      console.error("Login failed:", err);
      // keep current error handling (you already render error from mutation)
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-8 sm:p-10 md:p-12 flex flex-col justify-center">
              <div className="mb-8">
                ...
                <h2 className="text-2xl font-bold text-gray-800 mt-4 mb-1">Welcome back 👋</h2>
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/80 border border-gray-200 text-gray-700 placeholder:text-gray-300 outline-none focus:ring-2 focus:ring-pink-300 focus:border-pink-300 transition"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className="w-full pl-11 pr-11 py-3 rounded-xl bg-white/80 border border-gray-200 text-gray-700 placeholder:text-gray-300 outline-none focus:ring-2 focus:ring-purple-300 focus:border-purple-300 transition"
                    />
                    <button
                      type="button"
                ...
                {error && (
                  <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3">
                    <p className="text-red-400 text-sm">Login failed. Please check your credentials.</p>
                  </div>
                )}
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 hover:opacity-90 transition shadow-lg shadow-purple-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
