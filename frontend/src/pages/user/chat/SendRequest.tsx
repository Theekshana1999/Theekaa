...
const SendRequest: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetSentRequestsQuery(
    currentUserId,
    { skip: !currentUserId }
  );
  ...
  if (isError)
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3">
        ...
        <button onClick={refetch} className="text-xs text-indigo-500 underline">
          Try again
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      {toast && (
        <div
          className={`fixed top-5 right-5 px-4 py-2 rounded-xl shadow-lg z-50 text-white text-sm font-medium
          ${
            toast.type === "success"
        ...
        return (
          ...
          >
            ...
            <div className="flex gap-2 flex-wrap items-center">
              <span
                className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${badge}`}
              >
                {req.status}
              </span>
              ...
              {req.status === "accepted" && (
                <>
                  <button
                    onClick={() => handleOpenChat(receiver)}
                  ...
                  <button
                    onClick={() => handleBlock(receiver._id, req._id)}
                    disabled={isBusy}
              ...
              {req.status === "blocked" && (
                <button
                  onClick={() => handleUnblock(receiver._id, req._id)}
                  disabled={isBusy}
