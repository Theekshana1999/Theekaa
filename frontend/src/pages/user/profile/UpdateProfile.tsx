...
const UpdateProfile: React.FC = () => {
  ...
  // normalize rawUser (support { data: user } or user)
  const raw: any = rawUser;
  const user = raw?.data ?? raw ?? null;
  ...
  useEffect(() => {
    if (user) {
      ...
      setIncome(user.income != null ? String(user.income) : "");
  ...
  const input =
    "w-full border border-gray-200 rounded-xl p-3 outline-none focus:ring-2 focus:ring-pink-400 focus:border-pink-400 transition text-sm";
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50 flex items-center justify-center p-4">
      <div className="w-full max-w-3xl backdrop-blur-xl bg-white/80 border border-white/40 shadow-2xl rounded-2xl p-6 md:p-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div
                className={`w-full border rounded-xl p-3 text-sm flex items-center justify-between transition ${
                  age !== null ? "border-pink-200 bg-gradient-to-r from-pink-50 to-purple-50" : "border-gray-200 bg-gray-50"
                }`}
              >
                {age !== null ? <span className="font-semibold text-gray-700">{age} years old</span> : <span className="text-gray-300">Select date of birth</span>}
                {age !== null && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white shrink-0">{age} yrs</span>}
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {["Male", "Female"].map((g) => (
                <button
                  ...
                  type="button"
                  onClick={() => setGender(g)}
                  className={`p-3 rounded-xl border font-medium text-sm transition ${gender === g ? "border-pink-400 bg-pink-50 text-pink-600" : "border-gray-200 hover:border-pink-300"}`}
                >
                  {g}
                </button>
              ))}
          ...
          {/* Submit */}
          <button type="submit" className="w-full py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 hover:opacity-90 transition shadow-lg">
            Update Profile
          </button>
        </form>
      </div>
