...
const ProfileDetails: React.FC = () => {
  ...
  // pass string and skip when undefined to satisfy RTK Query generic typing
  const { data: rawUser, isLoading, error } = useGetUserQuery(userId ?? "", { skip: !userId });

  // normalize wrapper { data: user } or raw user object
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8">
            <div className="flex flex-col md:flex-row md:Items-center gap-6">
              ...
              <div>
                ...
                <p className="text-gray-500 mt-1">{user?.occupation || "No occupation added"}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
            {[
              ...
              <div key={label} className="flex flex-col">
                ...
                <span className="text-base font-medium text-gray-700 mt-1">{value || "Not provided"}</span>
                <div className="border-b border-purple-100 mt-3" />
              </div>
            ))}
          </div>
        </div>
